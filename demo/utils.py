...
def load_content(fname, mode="r"):
    ...
    with open(
        os.path.join(CONTENTS_DIR, fname), mode, errors=errors, encoding=encoding
    ) as f:
        return f.read()
...
def sidebar():
    with st.sidebar:
        ...
        st.markdown(
            "A RAG design pattern for secure, high-quality LLM-based question answering."
        )
        st.markdown("📖 [Read the paper](https://arxiv.org/abs/2508.02872)")
        st.markdown(
            '<img src="https://pngimg.com/d/github_PNG40.png" width="30"> [Code](https://github.com/microsoft/highlight-summarize)',
